...
rm -rf $GITHUB_WORKSPACE/wrt/feeds/luci/applications/luci-app-passwall
# git clone https://github.com/Openwrt-Passwall/openwrt-passwall $GITHUB_WORKSPACE/wrt/package/passwall-luci
...
git_sparse_clone() {
	...
	for folder in "$@"; do
		...
		del_dirs=$(find "$pwd_path/feeds/luci" "$pwd_path/feeds/packages" -maxdepth 3 -type d -exec bash -c '[[ "$(basename "$1")" == "'"$foldername"'" ]] && echo "$1"' _ {} \;)
        if [[ -n "$del_dirs" ]]; then
            while read -r dir; do
                rm -rf "$dir"
                echo "[delete] $dir"
...
if [[ $WRT_REPO == *"openwrt/openwrt"* ]]; then
	...
	git_sparse_clone master https://github.com/immortalwrt/packages package net/zerotier net/ddns-go net/chinadns-ng net/dns2socks net/tcping net/geoview lang/python/python-setuptools
